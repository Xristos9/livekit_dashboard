...
function extractPhoneFromRoomName(
...
function extractMp4Ref(json: any): {
  ...
  try {
    if (Array.isArray(json?.files) && json.files.length > 0) {
      ...
      const filename =
        typeof f?.filename === "string" ? f.filename.trim() : null;
      const location =
        typeof f?.location === "string" ? f.location.trim() : null;
...
export default defineEventHandler(async (event: H3Event) => {
  ...
  const jsonLimit = Math.max(
    1,
    Math.min(200, Number(q.jsonLimit ?? DEFAULT_JSON_LIMIT))
  ); // clamp 1..200
  ...
  async function getObjectTextWithTimeout(
    ...
    console.log(
      `[getObjectTextWithTimeout] Retrieved JSON: ${key}, length=${text.length}`
    );
    return text;
  }
  ...
  async function processOne(obj: Obj) {
    try {
      ...
      const startedAt =
        typeof data?.started_at === "number" ? data.started_at : null;
      const endedAt = typeof data?.ended_at === "number" ? data.ended_at : null;
      const roomName =
        typeof data?.room_name === "string" ? data.room_name : null;
  ...
  console.log(
    `[runPool] Starting pool with chunkSize=${chunkSize}, total=${jsonBatch.length}`
  );
